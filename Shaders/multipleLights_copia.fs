...
uniform float porcentajeMezcla = 0.3;
...
vec3 calculateDirectionalLight(Light light, vec3 direction){
	...
	// mix() es 70% el primero y 30% el segundo
	// vec3 finalColor = mix(textureColor1, textureColor2, 0.3);
	vec3 finalColor = mix(textureColor1, textureColor2, porcentajeMezcla);
